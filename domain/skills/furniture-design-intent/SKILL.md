...
1. 读取 [家具目录](references/intake/catalog.yaml)，由 LLM 根据用户完整语义把家具描述归一化为 `families` 中的规范 `furniture_type`；归一化判据见 [意图采集规则](references/intent-capture-rules.md)。无法可靠归类时只生成草稿 fallback，不确认、不进入可执行流水线。
...
3. 草稿尺寸可为 `null`；确认前只校验：类别已归一化、宽/深/高均为正数、吊柜挂装方式 `mount_mode` 完整（`free_height` 时挂高为正数）。挂装方式取值与挂高基准见 [意图采集规则](references/intent-capture-rules.md)。
4. 展示外包络与吊柜挂装方式并等待确认。门、层板、抽屉等进入布局阶段；板厚、背板、踢脚等进入板件阶段；材料、饰面、五金等进入制造阶段；不在本阶段捕获的完整清单见 [意图采集规则](references/intent-capture-rules.md)。
