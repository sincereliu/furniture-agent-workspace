...
1. 仅在用户明确要求房间摆放或摆放预览时运行；可读取已确认的家具类别与成品外包络（吊柜含底边离地挂高），也可接受独立请求中明确给出的同等输入。
2. 按 [空间布局规则](references/spatial-layout-rules.md) 生成用于摆放计算的 `CabinetLayout`，保留家具外包络；兼容字段中的 `shelf_count/door_count` 不参与房间定位，也不向主流程提供板件输入。
...
- 门数、层板数、抽屉数、隔板分区、开放格、滑门、挂衣区或设备区属于家具本体规划；其中已支持的 `shelf_count/n_doors/drawer_count` 进入 `panels_planned`，不得要求先生成房间摆放图。
