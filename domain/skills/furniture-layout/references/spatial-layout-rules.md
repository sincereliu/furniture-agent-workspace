...
- 位置：沿北墙居中，落地柜标高为 `0`；
- 吊柜：沿北墙居中；若已确认意图提供了挂高（底边离地高度 `mounting_height_mm`），则沿用该值作为 `origin_z_mm`，否则默认保留 `450 mm` 顶部净距；空间不足时降至不低于地面。
...
- 吊柜：已确认意图的挂高作为 `origin_z_mm` 继承；独立请求可显式提供 `origin_z_mm`，若为 0 则警告；挂墙结构和安装策略不进入 `CabinetLayout`。
