...
- 宽、深、高任一未知：先向客户要成品外包络，不要用常见柜尺寸填上再当事实。
- 吊柜未给出挂装方式：先追问是自由挂高还是贴顶；自由挂高还要底边离地高度。不要猜测。
- 展示草稿时逐项标明「用户已给」还是「假设」。
...
- 在外包络确认后停止并冻结为 JSON；不计算布局、净空、板件、制造策略、特征树或 CAD。下游规划只读冻结意图。
