...
- CLI/API 完整请求由 `furniture_workflow/input_adapter.py` 拆成 `DesignIntent` 与 `stage_inputs`。扁平的 `furniture_type`、宽深高、挂装方式和挂高映射为设计意图的规范字段；门数、板厚等下游参数写入对应阶段输入，不进入 `DesignIntent`。
