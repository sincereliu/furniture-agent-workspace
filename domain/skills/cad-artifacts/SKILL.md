...
5. API 生成请求显式提交完整板件字段；`back_mount/back_rail_height` 等值只路由到板件阶段。背板模式由 `panels_planned` 准入，API 返回制造备注、加工操作和 drilled-holes。
6. `workflow_artifact_writer.py` 写跨阶段快照；板件快照用已确认冻结板件，不用事后改过的内存副本。Orchestrator 不实现 JSON、BOM、孔位或 CAD 源码序列化。
