...
人给摆放意图，代码换毫米并做摆放检查；模块入口与两条产品线见 [运行时映射](runtime-map.md)。

## 坐标约定

- 默认毫米。件局部 `W×D×H` 对应 X 左→右、Y 后→前、Z 向上。
- 件局部原点 `(0,0,0)` 为外形尺寸左后下落地角。
...
边界接触不算干涉。可编辑视图的拖动按同一套摆放检查求解，干涉、越界或遮挡洞口就停在接触处（JS 侧镜像的判定在 `editor.py`，改 `placement_check.py` 要同步改它）。项目布局在创建待确认 Revision 前校验；缺房间尺寸或 fill 没有空段均失败。独立房间场景接口还要求非空 `items[]`。
