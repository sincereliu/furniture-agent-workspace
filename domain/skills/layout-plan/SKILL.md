...
3. 调用 `furniture_create_project`，传入项目 `name` 和 `rooms`。运行时计算几何、碰撞及 `wall + fill` 净长；无效布局直接返回错误，不创建项目。纯规划调用 `plan_project_layout(rooms)` 也执行同一校验。规则见 [空间布局规则](references/spatial-layout-rules.md)，可执行柜类见 [家具目录](references/intake/catalog.yaml)。
