...
- `null` / `auto` 只用于其自身语义：`gap_below_mm=null` 表示计算层，`toe_kick_support_count=null` 与 `back_mount=auto` 是显式结构化请求，不是“没问过”。
...
- 柜体/门板 18、背板 9、`back_mount=auto`、背板后移 18、前脸边距 1.5、铰链深度缝 2、槽深 6、槽余量 1、背拉条高 70、踢脚支撑 `null`。
- 抽屉每侧净空 13、层缝 1.5、底/背板厚 18、后净空 0。
- 落地柜：踢脚 50、前后退让 1/30、2 门、`drawer_count=0`、4 层固定层板。层板净高由 LLM 按内净高均分成数写入 `top_gap_mm` 与各层 `gap_below_mm`（运行时不均分）。
- 吊柜：踢脚 0、前后退让 0、2 门、`drawer_count=0`、1 层固定层板。同样由 LLM 把顶格与该层下净高等分成数写入。
