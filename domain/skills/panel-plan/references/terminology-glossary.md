...
| 内空 | `interior.cavity` | 对象 | 可用内空：`width/height/depth` 与最小角点 `origin.{x,y,z}`。不是缝隙 `clearance`。 |
| 内空分区 | `interior.zones[]` | 列表 | `kind` 为 `doors` 或 `full_height_drawers`；成员是门板 id 或抽屉装配 id。 |
...
| 板件接触 | `joints` | 列表 | 检查点写在所属子装配上；交接派生列表才回贴到板上。每条是大面被端面顶住。 |

## 接触口径

一条接触是定向面–端邻接：大面被端面顶住。角色属于这一条 `joints[]` 记录，不是板件属性。同一块板可以在不同接触里分别担任大面和端面。本阶段只推导接触，不解析连不连。输出键即下表规范名。

| 概念 | 规范名 | 单位/类型 | 说明 |
| --- | --- | --- | --- |
| 大面板件 | `bearing_id` | 标识符 | 拿出大面、被顶住的板。 |
| 端面板件 | `end_id` | 标识符 | 用厚度端顶住大面的板。 |
| 大面方向 | `face` | 面标记 | 大面所用语义面，值为该板 `inner_face`，如 `+x`。 |
...
- `interior` 在本阶段指柜体内空与分区，不指房间布局、踢脚区或背板基准。
