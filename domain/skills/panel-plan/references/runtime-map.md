...
| `joint_topology.py` | 大面–端面接触几何；不解析连不连 | calculation |
...
本阶段只产出尺寸、位置和大面–端面接触。接触字段是 `bearing_id`、`end_id`、`face`、`edge_axis`、`edge_sign`、`end_z`、`end_size_z`。接触上出现 `connection` 时拒绝加载。连不连由制造阶段自己决定。
