...
- `toe_kick_support_count` 必须是非负整数；运行时按该值生成支撑板，不从柜宽推断。
- 若 `toe_kick_height = 0`，则 `toe_kick_support_count` 必须为 `0`，且不生成踢脚支撑。
- 用户没说时，LLM 按候选起点写出整数：`W < 600 → 0`，否则 `1 + floor((W - 600) / 300)`。
