...
5. 依据 [背板结构规则](references/back-construction-rules.md)、[板件定义规则](references/panel-definition-rules.md)、[抽屉尺寸链](references/drawer-dimension-chain.md) 和 `references/cabinet-topologies/` 生成柜体实例及其 `spec/structure/back_mount_resolution/assemblies/openings`；当背板模式需要时，同时物化背拉条并纳入同一阶段校验。
