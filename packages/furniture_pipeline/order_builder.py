...
from furniture_schema.order import Order
...
from furniture_panelizer.cut_optimizer import (
    CuttingPlan,
    CutListReport,
    format_cut_list_markdown,
    optimize_cutting,
    render_cut_layout_png,
)
from furniture_panelizer.bom import BOMReport, format_bom_markdown
...
def build_order(order: Order, order_dir: Path) -> dict:
        ...
        for panel in result.panels:
            ...
            if panel.panel_type == "door":
                holes_data["hinge_holes"] = calc_hinge_positions(
                    door_height_mm=panel.size_z,
                    door_width_mm=panel.size_x,
    ...
    drilling_json.write_text(
        json.dumps(all_drilling, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    ...
    return {
        ...
        "purchase": [
            str(hardware_json),
            str(hardware_md),
            str(panel_purchase_json),
            str(panel_purchase_md),
        ],
...
def _write_hardware_files(
    hw_list: List[HardwareRecord], json_path: Path, md_path: Path
):
...
def _write_cut_files(
    report: CutListReport, json_path: Path, md_path: Path
):
    json_path.write_text(
        json.dumps(
            [
...
def _write_panel_files(
    panels: List[PanelRecord], json_path: Path, md_path: Path
):
    json_path.write_text(
        json.dumps(
            [
