...
- 完整参数包括数量 `shelf_count/n_doors/drawer_count` 与单门铰链侧 `door_hinge_side`（`n_doors=1` 时必填 `left/right`，其余显式 `null`），主体厚度，背板模式/偏移/槽/背拉条，门缝与踢脚，以及五项抽屉净空/厚度；单位均为 mm。自然语言说明留在交互中。
