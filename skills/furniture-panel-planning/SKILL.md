...
3. 本阶段按有效 `back_mount` 生成入槽、内嵌或外盖背板；仅入槽模式生成背拉条。本阶段同时拥有前后踢脚板位置以及支撑板数量、尺寸和等距位置。
4. 通过 `FurnitureOrchestrator.run_next()` 生成板件阶段输出，并由 `scripts/furniture_panel_planning/validation.py` 校验板件标识、尺寸、位置、依赖和背板几何。
