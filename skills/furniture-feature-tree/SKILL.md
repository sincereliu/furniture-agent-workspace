...
description: 用于 feature_tree_planned 阶段；将已确认制造策略转为可审查的部件、依赖、顺序和 CAD 建模语义，不生成几何。
---
...
2. 按 [特征树规则](references/feature-tree.md) 转换建模职责、依赖和顺序。
3. Feature Tree v2 用 `box` 表示板件，用带 `target` 的 `cut_box` 表示切削；目标须已存在，切削须完全位于其包络内。
