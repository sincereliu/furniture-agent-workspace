...
4. 打孔和匹配逻辑集中在 `connectors/` 模块中（基类 `Connector`，子类 `TrinityConnector` / `HingeConnector` / `ShelfConnector`）。新增五金只需新建一个 Connector 文件并注册到 `ALL_CONNECTORS`，无需改动其他代码。
5. `manufacturing_bom.py` 的 `estimate_hardware()` 遍历 `ALL_CONNECTORS` 生成 BOM；`emit_drilled_holes()` 遍历 `ALL_CONNECTORS` 生成孔位 JSON（含全局坐标和板自身 local 坐标）；`drilled_holes_glb.py` 将孔位导出为彩色 GLB 标记。
6. 入槽背板生成左侧板、右侧板、顶板、底板 4 条目标明确的 `cut_box` 加工记录；槽宽 = `back_thickness + groove_clearance`，槽深 = `groove_depth`。
7. 材料厚度必须来自确认后的 `FurnitureSpec`，不得用硬编码厚度覆盖用户输入。
8. 门板支持可选字段 `door_hinge_side`（"left"/"right"），未指定时由 `HingeConnector` 根据面板位置自动推断。
9. 通过 `FurnitureOrchestrator.run_next()` 生成制造阶段输出，展示后暂停。
