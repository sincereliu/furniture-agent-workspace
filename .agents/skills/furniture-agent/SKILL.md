...
独立能力（不在上述串联阶段内）：
   ...
   - 房间摆放、碰撞检查、SVG/互动 Viewer：`skills/furniture-layout/SKILL.md`
...
- 仅明确的一次性家具生成批处理可用 `skills/furniture-cad/scripts/generate_furniture.py` 或 `execute_spec()`；`server.py` 的家具生成端点只适配协议并调用 Orchestrator，独立 `/api/plan-layout` 端点调用 `furniture-layout` 自有运行时。
...
- 禁止根级 `scripts/`、`packages/`、`tests/`、`scratch/`、`tmp/`；生成源码不得进入 `generated/`。工作区目录变更后运行 `skills/furniture-cad/scripts/validate_workspace_layout.py` 并清零违规。
